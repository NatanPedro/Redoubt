...
**Cifrar para um destinatário (X25519).** Cada instalação tem um par de chaves de destinatário. *Segurança ▸ Exportar minha chave de destinatário* copia a sua **chave pública**; *Segurança ▸ Selar para destinatário* cifra a aba **para a pública de outra pessoa** (estilo `age`) — e também para você, pra manter acesso. Abrir um `.rdbt` selado para você é **automático**: a chave local destrava sem senha. E se quiser fechar o cerco, *Segurança ▸ Proteger chave de destinatário com senha* embrulha a sua privada num Cofre (a pública fica em claro, então exportar e selar seguem sem senha; só **abrir** pede a credencial, 1× por sessão). Como no `age`, isso **não autentica o remetente**.
