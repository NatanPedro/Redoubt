...
> | --- | --- | --- |
> | **Confidencialidade em repouso** | Cofre `.rdbt` (AES-256-GCM · Argon2id · X25519) | quem não tem a credencial nem a chave de destinatário não lê o conteúdo |
...
### Formato RDBT4 (envelope / key-slots, estilo LUKS/age)

- Uma **chave-de-conteúdo (CK)** aleatória de 256 bits cifra o texto com **AES-256-GCM**
  (cifragem autenticada).
- Cada **destravador** é um **slot** (de tamanho variável, *length-prefixed*) que **embrulha a CK**. Três tipos:
  - **senha** ou **arquivo-chave** → KEK derivada por **Argon2id** (*memory-hard*; lê **scrypt** de cofres legados).
  - **destinatário X25519** → KEK por **ECDH efêmero + HKDF** — cifrar **para a chave pública** de alguém (estilo `age`).
- **Múltiplos destravadores independentes** (senhas, arquivos-chave e/ou destinatários) abrem o **mesmo** cofre (até
  **16 slots**); tipos diferentes **coexistem**. Re-selar preserva todos os slots; a CK fica em memória.
- O **AAD** liga o conteúdo a **todos** os slots (anti slot-strip) e o byte `kind` à CK embrulhada (**anti-downgrade**).
  O custo de KDF tem **teto por slot e agregado** (anti-DoS: ~minutos/512 MiB → ~5s/128 MiB). Slot inválido / KDF
  desconhecido / **ponto X25519 de ordem baixa** / payload malformado é **pulado** — nunca crasha nem nega credencial válida.
- Lê **RDBT3/RDBT2** (slots fixos de 80 bytes, re-enquadrados sem reembrulhar — a AAD do slot é preservada) e **migra**
  o **RDBT1** legado; re-selar atualiza o envelope para RDBT4.

### Cifrar para um destinatário (X25519)

Cada instalação tem um **par X25519 de destinatário** (`recipient.x25519`, local). *Segurança ▸ Exportar minha chave de
destinatário* compartilha a **pública**; *Selar para destinatário* cifra o conteúdo **para a pública de alguém** (e também
para você, p/ manter acesso). Abrir um `.rdbt` selado para você é **automático** — a chave X25519 local é tentada antes da
senha (e só se ela **já existe**: abrir um cofre de terceiro não materializa uma chave sua).
